...
/* This script can easily be generalized for any kind of nights observing goals.
	...
	The script does allow for one "retry", that is when there are remaining objects, but they 
...
var exposureTime = 3.0;			// Number of seconds for each exposure
var exposureDelay = 1.0;			// Delay between exposures
var ditherArcSeconds = 10.0;		// Dither amount per exposure (in arcseconds)
var focusEveryXObjects = 500;	// Refocus every so many targets (just make arbitrarily large to skip)
var minAltitude = 30.0;				// Minumum altitude at which something can be imaged
var waitRetryDelay = 60*60;		// If we run out of targets, wait this long (in seconds) and try clearing the list again
...
function findNextCandidate()
	...
	for(i = 0; i < Objects.length; i++) {
     ...
     sky6ObjectInformation.Property(sky6ObjInfoProp_DEC_NOW);
		...
		if(nBestObjectIndex < 0){	// Seeking first object above the horizon
