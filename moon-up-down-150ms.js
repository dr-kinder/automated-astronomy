...
var DataName = "moon-up-down-150ms"; // Give the output files a logical name.
...
// Up = +Dec
var StartTime = GetTime();
Mount.SlewToRaDec(MoonRA, MoonDec + DecOffset, DataName + "-u0");
var EndTime = GetTime();
var UUSlewTime = TimeDifference(StartTime,EndTime);
logOutput(TargetName + "_U0" + " | Slew time: " + UUSlewTime[4] + ":" + UUSlewTime[5]);
TakeMultiPhoto(FrameCount, ExposureTime);

// Down = -Dec
var StartTime = GetTime();
Mount.SlewToRaDec(MoonRA, MoonDec - DecOffset, DataName + "-d0");
var EndTime = GetTime();
var DDSlewTime = TimeDifference(StartTime,EndTime);
logOutput(TargetName + "_D0" + " | Slew time: " + DDSlewTime[4] + ":" + DDSlewTime[5]);
TakeMultiPhoto(FrameCount, ExposureTime);
...
logOutput("0L: " + UUSlewTime[4] + ":" + UUSlewTime[5]);
logOutput("0R: " + DDSlewTime[4] + ":" + DDSlewTime[5]);
...
// ----------------------------------------------------------------------------- 
logOutput("150ms up-down protocol running.");
